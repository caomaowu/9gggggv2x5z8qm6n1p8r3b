...
CONSTRAINED_PROMPT_TEMPLATE = """你是一名专业的量化交易分析师，正在分析{stock_name}的{time_frame}K线图。你的任务是发布**立即执行指令**：**做多**或**做空**。在输出最终JSON之前，必须进行严谨的深度思考与内部推理，最终只输出JSON结果。
            ...
            **重要：你必须基于当前价格 {latest_price_str} 来计算具体的止损止盈点位！**
            ...
            如果分析报告包含多个时间周期的数据，请遵循以下策略：
            
            ### 1. 周期层级决策框架：
            - **长周期（日线/4小时）**：确定主趋势方向和市场阶段
              - 上升趋势：优先考虑做多机会
              - 下降趋势：优先考虑做空机会
              - 震荡区间：等待突破确认
            
            - **中周期（1小时/30分钟）**：评估趋势强度和回调/反弹幅度
              - 验证长周期趋势的健康度
              - 识别回调/反弹的支撑阻力位
              - 判断是否有趋势转折迹象
            
            - **短周期（15分钟/5分钟）**：寻找具体入场时机
              - 在长周期方向一致时寻找短期信号
              - 识别精确的入场点位
              - 设置更紧密的止损位
            ...
            ### 5. 综合决策原则：
            1. **长周期定方向** - 永远不逆长周期主趋势
            2. **中周期看强度** - 评估趋势健康度和延续性
            3. **短周期找时机** - 精确入场和快速止损
            4. **共振才重仓** - 多周期一致时可提高仓位
            5. **分歧要轻仓** - 周期矛盾时降低仓位或观望
            ...
            **justification 字段必须包含以下详细分析内容（以结论摘要形式呈现，不泄露内部推理过程）：**
            ...
            1. **市场环境分析依据**
               ...
               - **如果是多时间框架**：说明各个时间框架的市场环境判断，识别长短周期的一致性或分歧

            2. **技术指标信号解读**
               - 具体分析MACD、RSI、Stochastic等关键指标
               - 说明指标间的相互验证或矛盾
               - 解读指标信号的强度和可靠性
               - **如果是多时间框架**：对比不同时间框架的指标表现，识别多周期指标共振或背离

            3. **形态识别分析**
               - 识别出的具体形态及其完成度
               - 形态的突破确认情况
               - 形态对后续走势的预示意义
               - **如果是多时间框架**：说明各个时间框架的形态特征，形态在不同周期的确认度

            4. **趋势线判断**
               - 支撑线/阻力线的有效性分析
               - 价格与趋势线的交互情况
               - 趋势线对未来走势的指导意义
               - **如果是多时间框架**：多周期趋势方向的一致性评估，关键支撑阻力位的多周期验证
            ...
            5. **综合决策逻辑**
               ...
               - **如果是多时间框架**：
                 - **多周期共振分析**：说明哪些时间框架信号一致
                 - **周期权重分配**：长周期方向 + 短周期时机
                 - **分歧处理说明**：如有周期矛盾，说明如何权衡
