...
- **如果是多时间框架**：优先考虑长周期形态的方向指引，短周期形态用于确认突破时机

趋势报告指导：
- 分析价格与支撑/阻力的互动：向上倾斜的支撑线→买盘兴趣；向下倾斜的阻力线→卖压
- 若价格在趋势线之间压缩，仅在存在强蜡烛或指标确认的汇合时预测突破
- 不要仅凭几何形状假设突破方向
- **如果是多时间框架**：长周期趋势线确定主方向，短周期趋势线用于设置精确的止损点位
...
7. 止盈止损：必须给出数值型 stop_loss 与 take_profit；stop_loss 不得等于当前价格；结合市场环境与波动性给出风险控制建议，并按品种精度四舍五入
   **如果是多时间框架**：
   - 止损位：参考短周期关键结构位，结合长周期支撑/阻力
   - 止盈位：优先考虑长周期的技术目标位，分批止盈策略
8.justification 字段必须提供详细分析，不能敘衍或过于简单。必须说明决策的具体依据和推理逻辑，并引用关键数值与价格位
   **如果是多时间框架**：必须说明各个时间框架的信号一致性，长周期方向与短周期时机的结合逻辑
9.止盈空间必须大于止损空间
