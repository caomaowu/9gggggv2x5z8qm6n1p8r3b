...
class CsvWriter:
    ...
    旧实现把数据交给 daemon 写线程后立即返回，写线程打开文件失败、运行中
    异常或停止超时时，主流程仍会显示完成。这里在调用线程内串行写入；write()
    成功返回即代表该行已经交给操作系统，stop() 再完成最终落盘确认。
    """
    ...
    def write(self, row: Dict[str, Any]) -> None:
        with self._lock:
            ...
            try:
                ...
                # 每行 flush，避免高并发结果洪峰后界面进度领先于文件内容。
                self._file.flush()
                self.rows_written += 1
...
def run_tasks_concurrently(
    ...
    *,
    ...
    with ThreadPoolExecutor(max_workers=worker_count, thread_name_prefix="backtest") as executor:
        # 只维持 worker_count 个在途任务，不再一次性创建全部 Future。
        ...
        while pending:
            ...
            for future in done:
                ...
                try:
                    ...
                    # 一个任务在正常错误处理之外崩溃时，也必须产出一条结果，
                    # 否则 CSV 行数会少于任务数，且整批可能提前中断。
